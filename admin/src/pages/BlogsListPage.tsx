import { useEffect, useState } from "react"
import { Link } from "react-router-dom"
import { Plus, Pencil, Trash2, Search } from "lucide-react"
import { useAuth } from "@/context/AuthContext"
import { deleteBlog, fetchAdminBlogs, type AdminBlog } from "@/lib/api"
import { PageHeader } from "@/components/FormField"

export default function BlogsListPage() {
  const { token } = useAuth()
  const [blogs, setBlogs] = useState<AdminBlog[]>([])
  const [status, setStatus] = useState("all")
  const [q, setQ] = useState("")
  const [error, setError] = useState("")
  const [loading, setLoading] = useState(true)

  async function load() {
    if (!token) return
    setLoading(true)
    setError("")
    try {
      const list = await fetchAdminBlogs(token, {
        status: status === "all" ? undefined : status,
        q: q.trim() || undefined,
      })
      setBlogs(list)
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to load blogs")
    } finally {
      setLoading(false)
    }
  }

  useEffect(() => {
    void load()
  }, [token, status])

  async function onDelete(blog: AdminBlog) {
    if (!token) return
    if (!confirm(`Delete “${blog.title}”? This cannot be undone.`)) return
    try {
      await deleteBlog(token, blog.id)
      setBlogs((prev) => prev.filter((b) => b.id !== blog.id))
    } catch (err) {
      setError(err instanceof Error ? err.message : "Delete failed")
    }
  }

  return (
    <div className="admin-page">
      <PageHeader
        title="Blogs"
        description="Technical guides shown on the public /resources pages."
        action={
          <Link to="/blogs/new" className="admin-btn admin-btn-primary">
            <Plus className="size-4" aria-hidden="true" />
            Add blog
          </Link>
        }
      />

      <div className="admin-toolbar">
        <div className="relative flex-1">
          <Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-steel" aria-hidden="true" />
          <input
            value={q}
            onChange={(e) => setQ(e.target.value)}
            onKeyDown={(e) => e.key === "Enter" && void load()}
            placeholder="Search by title, slug or category…"
            className="admin-input pl-9"
          />
        </div>
        <select value={status} onChange={(e) => setStatus(e.target.value)} className="admin-input sm:w-44">
          <option value="all">All statuses</option>
          <option value="published">Published</option>
          <option value="draft">Draft</option>
        </select>
        <button type="button" onClick={() => void load()} className="admin-btn admin-btn-secondary">
          Search
        </button>
      </div>

      {error && (
        <p role="alert" className="mb-4 rounded-xl bg-red-50 px-3 py-2 text-sm text-red-700">
          {error}
        </p>
      )}

      <div className="admin-table-wrap">
        <table>
          <thead>
            <tr>
              <th>Title</th>
              <th>Category</th>
              <th>Status</th>
              <th>Date</th>
              <th className="text-right">Actions</th>
            </tr>
          </thead>
          <tbody>
            {loading && (
              <tr>
                <td colSpan={5} className="!py-10 text-center text-steel">
                  Loading…
                </td>
              </tr>
            )}
            {!loading && blogs.length === 0 && (
              <tr>
                <td colSpan={5} className="!py-10 text-center text-steel">
                  No blogs found. Seed the database or add a blog.
                </td>
              </tr>
            )}
            {!loading &&
              blogs.map((b) => (
                <tr key={b.id}>
                  <td>
                    <p className="font-bold text-charcoal">{b.title}</p>
                    <p className="text-xs text-steel">/resources/{b.slug}</p>
                  </td>
                  <td className="text-steel">{b.category}</td>
                  <td>
                    <span
                      className={`admin-badge ${
                        b.status === "published" ? "admin-badge-published" : "admin-badge-draft"
                      }`}
                    >
                      {b.status}
                    </span>
                  </td>
                  <td className="text-steel">{b.date || "—"}</td>
                  <td>
                    <div className="flex justify-end gap-2">
                      <Link to={`/blogs/${b.id}/edit`} className="admin-btn admin-btn-secondary !px-2.5 !py-1.5 text-xs">
                        <Pencil className="size-3.5" aria-hidden="true" />
                        Edit
                      </Link>
                      <button
                        type="button"
                        onClick={() => void onDelete(b)}
                        className="inline-flex items-center gap-1 rounded-lg border border-red-200 px-2.5 py-1.5 text-xs font-bold text-red-700 hover:bg-red-50"
                      >
                        <Trash2 className="size-3.5" aria-hidden="true" />
                        Delete
                      </button>
                    </div>
                  </td>
                </tr>
              ))}
          </tbody>
        </table>
      </div>
    </div>
  )
}
