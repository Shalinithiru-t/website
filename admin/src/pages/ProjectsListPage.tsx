...
import { deleteProject, fetchAdminProjects, type AdminProject } from "@/lib/api"
...
export default function ProjectsListPage() {
  ...
  async function load() {
    ...
    setError("")
    try {
      const list = await fetchAdminProjects(token, {
  ...
  async function onDelete(item: AdminProject) {
    ...
    try {
      await deleteProject(token, item.id)
  ...
  return (
    <div className="admin-page">
      <PageHeader
        title="Projects"
        description="Case studies on the public Projects page."
        action={
              ...
              items.map((p) => (
                <tr key={p.id}>
                  <td>
                    ...
                    <p className="text-xs text-steel">/projects/{p.slug}</p>
                  </td>
                  <td className="text-sm text-steel">
                    {p.city}, {p.state}
                  </td>
                  <td className="text-sm text-steel">{p.applicationFilter}</td>
                  <td>
                    <span
                  ...
                  <td>
                    <div className="flex justify-end gap-2">
                      <Link to={`/projects/${p.id}/edit`} className="admin-btn admin-btn-secondary !px-2.5 !py-1.5 text-xs">
