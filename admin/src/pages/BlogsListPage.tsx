...
import { deleteBlog, fetchAdminBlogs, type AdminBlog } from "@/lib/api"
...
export default function BlogsListPage() {
  ...
  useEffect(() => {
    void load()
    // intentionally reload when status/token change
  }, [token, status])
    ...
    <div className="p-8">
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-steel">Manage technical guides shown on /resources</p>
        </div>
        <Link
          to="/blogs/new"
        ...
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            ...
            {!loading && blogs.length === 0 && (
              <tr>
                <td colSpan={5} className="px-4 py-8 text-center text-steel">
                  No blogs found. Seed the database or add a blog.
                </td>
              </tr>
            )}
            {!loading &&
              blogs.map((b) => (
                <tr key={b.id} className="border-b border-border-grey last:border-0">
                  <td className="px-4 py-3">
                    <p className="font-medium text-charcoal">{b.title}</p>
                    <p className="text-xs text-steel">/{b.slug}</p>
                  </td>
                  <td className="px-4 py-3 text-steel">{b.category}</td>
                  <td className="px-4 py-3">
                    <span
                      className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${
                        b.status === "published" ? "bg-green-50 text-green-700" : "bg-amber-50 text-amber-700"
                      }`}
                    >
                      {b.status}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-steel">{b.date || "—"}</td>
                  <td className="px-4 py-3">
